import axios, { AxiosInstance } from 'axios';
import https from 'https';
import { BaseIntegration, ConnectionTestResult, MetricInfo, ApiCapability } from './base';
import {
  IntegrationConfig,
  IntegrationData,
  RadarrConfig,
  RadarrSystemStatus,
  RadarrDiskSpace,
  RadarrMovie,
  RadarrQueueRecord,
  RadarrHistoryRecord,
  RadarrHealth,
} from '../types';
import { logger } from '../services/logger';

export class RadarrIntegration extends BaseIntegration {
  readonly type = 'radarr';
  readonly name = 'Radarr';

  private createClient(config: RadarrConfig): AxiosInstance {
    const protocol = config.verifySSL ? 'https' : 'http';
    const baseURL = `${protocol}://${config.host}:${config.port || 7878}/api/v3`;

    return axios.create({
      baseURL,
      headers: {
        'X-Api-Key': config.apiKey,
        'Accept': 'application/json',
      },
      httpsAgent: new https.Agent({
        rejectUnauthorized: config.verifySSL ?? false,
      }),
      timeout: 30000,
    });
  }

  async testConnection(config: IntegrationConfig): Promise<ConnectionTestResult> {
    const radarrConfig = config as RadarrConfig;

    if (!radarrConfig.host) {
      return { success: false, message: 'Host is required' };
    }
    if (!radarrConfig.apiKey) {
      return { success: false, message: 'API key is required' };
    }

    try {
      const client = this.createClient(radarrConfig);
      const [statusResponse, movieResponse] = await Promise.all([
        client.get('/system/status'),
        client.get('/movie'),
      ]);

      const status = statusResponse.data;
      const movies = movieResponse.data;

      return {
        success: true,
        message: `Connected to Radarr v${status.version}`,
        details: {
          version: status.version,
          branch: status.branch,
          movieCount: movies.length,
          osName: status.osName,
        },
      };
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : String(error);
      logger.error('radarr', 'Connection test failed', { error: errorMsg });

      if (axios.isAxiosError(error)) {
        if (error.response?.status === 401) {
          return {
            success: false,
            message: 'Authentication failed: Invalid API key',
          };
        }
        if (error.code === 'ECONNREFUSED') {
          return {
            success: false,
            message: `Connection refused: Cannot reach ${radarrConfig.host}:${radarrConfig.port || 7878}`,
          };
        }
        if (error.code === 'ETIMEDOUT' || error.code === 'ECONNABORTED') {
          return {
            success: false,
            message: 'Connection timed out',
          };
        }
      }

      return {
        success: false,
        message: `Connection failed: ${errorMsg}`,
      };
    }
  }

  async getData(config: IntegrationConfig, metric: string): Promise<IntegrationData> {
    const radarrConfig = config as RadarrConfig;
    const client = this.createClient(radarrConfig);

    switch (metric) {
      case 'system-status':
        return this.getSystemStatus(client);
      case 'movies':
        return this.getMovies(client);
      case 'calendar':
        return this.getCalendar(client);
      case 'queue':
        return this.getQueue(client);
      case 'wanted':
        return this.getWantedMissing(client);
      case 'history':
        return this.getHistory(client);
      default:
        throw new Error(`Unknown metric: ${metric}`);
    }
  }

  private async getSystemStatus(client: AxiosInstance): Promise<{
    systemStatus: RadarrSystemStatus;
    diskSpace: RadarrDiskSpace[];
    health: RadarrHealth[];
  }> {
    try {
      const [statusResponse, diskResponse, healthResponse] = await Promise.all([
        client.get('/system/status'),
        client.get('/diskspace'),
        client.get('/health'),
      ]);

      return {
        systemStatus: statusResponse.data,
        diskSpace: diskResponse.data,
        health: healthResponse.data,
      };
    } catch (error) {
      logger.error('radarr', 'Failed to get system status', { error });
      throw error;
    }
  }

  private async getMovies(client: AxiosInstance): Promise<{ movies: RadarrMovie[] }> {
    try {
      const response = await client.get('/movie');
      return { movies: response.data };
    } catch (error) {
      logger.error('radarr', 'Failed to get movies', { error });
      throw error;
    }
  }

  private async getCalendar(client: AxiosInstance): Promise<{ movies: RadarrMovie[] }> {
    try {
      // Get movies for past 7 days and next 30 days
      const now = new Date();
      const start = new Date(now);
      start.setDate(start.getDate() - 7);
      const end = new Date(now);
      end.setDate(end.getDate() + 30);

      const response = await client.get('/calendar', {
        params: {
          start: start.toISOString(),
          end: end.toISOString(),
          unmonitored: false,
        },
      });

      return { movies: response.data };
    } catch (error) {
      logger.error('radarr', 'Failed to get calendar', { error });
      throw error;
    }
  }

  private async getQueue(client: AxiosInstance): Promise<{
    queue: RadarrQueueRecord[];
    totalRecords: number;
  }> {
    try {
      const response = await client.get('/queue', {
        params: {
          page: 1,
          pageSize: 50,
          includeMovie: true,
        },
      });

      return {
        queue: response.data.records || [],
        totalRecords: response.data.totalRecords || 0,
      };
    } catch (error) {
      logger.error('radarr', 'Failed to get queue', { error });
      throw error;
    }
  }

  private async getWantedMissing(client: AxiosInstance): Promise<{
    missing: RadarrMovie[];
    totalRecords: number;
  }> {
    try {
      // Get movies that are monitored but don't have a file
      const response = await client.get('/movie', {
        params: {},
      });

      // Filter to only missing (monitored without file)
      const missing = (response.data as RadarrMovie[]).filter(
        (movie) => movie.monitored && !movie.hasFile
      );

      return {
        missing: missing.slice(0, 50),
        totalRecords: missing.length,
      };
    } catch (error) {
      logger.error('radarr', 'Failed to get wanted/missing', { error });
      throw error;
    }
  }

  private async getHistory(client: AxiosInstance): Promise<{
    records: RadarrHistoryRecord[];
    totalRecords: number;
  }> {
    try {
      const response = await client.get('/history', {
        params: {
          page: 1,
          pageSize: 50,
          sortKey: 'date',
          sortDirection: 'descending',
          includeMovie: true,
        },
      });

      return {
        records: response.data.records || [],
        totalRecords: response.data.totalRecords || 0,
      };
    } catch (error) {
      logger.error('radarr', 'Failed to get history', { error });
      throw error;
    }
  }

  getAvailableMetrics(): MetricInfo[] {
    return [
      {
        id: 'system-status',
        name: 'System Status',
        description: 'Server info, disk space, and health',
        widgetTypes: ['radarr-status'],
      },
      {
        id: 'movies',
        name: 'Movie Library',
        description: 'All movies with file statistics',
        widgetTypes: ['radarr-movies'],
      },
      {
        id: 'calendar',
        name: 'Calendar',
        description: 'Upcoming and recent releases',
        widgetTypes: ['radarr-calendar'],
      },
      {
        id: 'queue',
        name: 'Download Queue',
        description: 'Active downloads with progress',
        widgetTypes: ['radarr-queue'],
      },
      {
        id: 'wanted',
        name: 'Wanted/Missing',
        description: 'Missing movies',
        widgetTypes: ['radarr-wanted'],
      },
      {
        id: 'history',
        name: 'History',
        description: 'Recent activity and downloads',
        widgetTypes: ['radarr-history'],
      },
    ];
  }

  getApiCapabilities(): ApiCapability[] {
    return [
      // System - Implemented
      {
        id: 'system-status',
        name: 'Get System Status',
        description: 'Get Radarr system status and version info',
        method: 'GET',
        endpoint: '/system/status',
        implemented: true,
        category: 'System',
        documentationUrl: 'https://radarr.video/docs/api/',
      },
      {
        id: 'health',
        name: 'Get Health',
        description: 'Get system health check results',
        method: 'GET',
        endpoint: '/health',
        implemented: true,
        category: 'System',
      },
      {
        id: 'disk-space',
        name: 'Get Disk Space',
        description: 'Get disk space for root folders',
        method: 'GET',
        endpoint: '/diskspace',
        implemented: true,
        category: 'System',
      },
      {
        id: 'system-backup',
        name: 'List Backups',
        description: 'Get list of available backups',
        method: 'GET',
        endpoint: '/system/backup',
        implemented: false,
        category: 'System',
      },
      {
        id: 'system-restart',
        name: 'Restart Radarr',
        description: 'Restart the Radarr service',
        method: 'POST',
        endpoint: '/system/restart',
        implemented: false,
        category: 'System',
      },

      // Movies - Implemented
      {
        id: 'movie-list',
        name: 'List Movies',
        description: 'Get all movies',
        method: 'GET',
        endpoint: '/movie',
        implemented: true,
        category: 'Movies',
      },
      {
        id: 'movie-get',
        name: 'Get Movie',
        description: 'Get a specific movie by ID',
        method: 'GET',
        endpoint: '/movie/{id}',
        implemented: false,
        category: 'Movies',
        parameters: [
          { name: 'id', type: 'number', required: true, description: 'Movie ID' },
        ],
      },
      {
        id: 'movie-add',
        name: 'Add Movie',
        description: 'Add a new movie',
        method: 'POST',
        endpoint: '/movie',
        implemented: false,
        category: 'Movies',
      },
      {
        id: 'movie-update',
        name: 'Update Movie',
        description: 'Update an existing movie',
        method: 'PUT',
        endpoint: '/movie/{id}',
        implemented: false,
        category: 'Movies',
      },
      {
        id: 'movie-delete',
        name: 'Delete Movie',
        description: 'Delete a movie and optionally files',
        method: 'DELETE',
        endpoint: '/movie/{id}',
        implemented: false,
        category: 'Movies',
        parameters: [
          { name: 'deleteFiles', type: 'boolean', required: false, description: 'Also delete files' },
          { name: 'addImportExclusion', type: 'boolean', required: false, description: 'Add to exclusion list' },
        ],
      },
      {
        id: 'movie-lookup',
        name: 'Search Movies',
        description: 'Search for movies to add',
        method: 'GET',
        endpoint: '/movie/lookup',
        implemented: false,
        category: 'Movies',
        parameters: [
          { name: 'term', type: 'string', required: true, description: 'Search term' },
        ],
      },
      {
        id: 'movie-lookup-tmdb',
        name: 'Lookup by TMDB',
        description: 'Lookup movie by TMDB ID',
        method: 'GET',
        endpoint: '/movie/lookup/tmdb',
        implemented: false,
        category: 'Movies',
        parameters: [
          { name: 'tmdbId', type: 'number', required: true, description: 'TMDB ID' },
        ],
      },
      {
        id: 'movie-lookup-imdb',
        name: 'Lookup by IMDB',
        description: 'Lookup movie by IMDB ID',
        method: 'GET',
        endpoint: '/movie/lookup/imdb',
        implemented: false,
        category: 'Movies',
        parameters: [
          { name: 'imdbId', type: 'string', required: true, description: 'IMDB ID' },
        ],
      },

      // Movie Files
      {
        id: 'moviefile-list',
        name: 'List Movie Files',
        description: 'Get movie files for a movie',
        method: 'GET',
        endpoint: '/moviefile',
        implemented: false,
        category: 'Movies',
        parameters: [
          { name: 'movieId', type: 'number', required: true, description: 'Movie ID' },
        ],
      },
      {
        id: 'moviefile-delete',
        name: 'Delete Movie File',
        description: 'Delete a movie file',
        method: 'DELETE',
        endpoint: '/moviefile/{id}',
        implemented: false,
        category: 'Movies',
      },

      // Calendar - Implemented
      {
        id: 'calendar',
        name: 'Get Calendar',
        description: 'Get upcoming and recent releases',
        method: 'GET',
        endpoint: '/calendar',
        implemented: true,
        category: 'Calendar',
        parameters: [
          { name: 'start', type: 'string', required: false, description: 'Start date (ISO 8601)' },
          { name: 'end', type: 'string', required: false, description: 'End date (ISO 8601)' },
        ],
      },

      // Queue - Implemented
      {
        id: 'queue-list',
        name: 'Get Queue',
        description: 'Get download queue',
        method: 'GET',
        endpoint: '/queue',
        implemented: true,
        category: 'Queue',
      },
      {
        id: 'queue-details',
        name: 'Get Queue Details',
        description: 'Get detailed queue with pagination',
        method: 'GET',
        endpoint: '/queue/details',
        implemented: false,
        category: 'Queue',
      },
      {
        id: 'queue-delete',
        name: 'Remove from Queue',
        description: 'Remove item from download queue',
        method: 'DELETE',
        endpoint: '/queue/{id}',
        implemented: false,
        category: 'Queue',
        parameters: [
          { name: 'blocklist', type: 'boolean', required: false, description: 'Add to blocklist' },
        ],
      },

      // Wanted - Implemented
      {
        id: 'wanted-missing',
        name: 'Get Missing Movies',
        description: 'Get missing movies (wanted)',
        method: 'GET',
        endpoint: '/wanted/missing',
        implemented: true,
        category: 'Wanted',
      },
      {
        id: 'wanted-cutoff',
        name: 'Get Cutoff Unmet',
        description: 'Get movies not meeting quality cutoff',
        method: 'GET',
        endpoint: '/wanted/cutoff',
        implemented: false,
        category: 'Wanted',
      },

      // History - Implemented
      {
        id: 'history-list',
        name: 'Get History',
        description: 'Get activity history',
        method: 'GET',
        endpoint: '/history',
        implemented: true,
        category: 'History',
      },
      {
        id: 'history-movie',
        name: 'Get Movie History',
        description: 'Get history for a specific movie',
        method: 'GET',
        endpoint: '/history/movie',
        implemented: false,
        category: 'History',
        parameters: [
          { name: 'movieId', type: 'number', required: true, description: 'Movie ID' },
        ],
      },

      // Commands
      {
        id: 'command-list',
        name: 'List Commands',
        description: 'Get running/queued commands',
        method: 'GET',
        endpoint: '/command',
        implemented: false,
        category: 'Commands',
      },
      {
        id: 'command-run',
        name: 'Run Command',
        description: 'Execute a command (scan, search, etc.)',
        method: 'POST',
        endpoint: '/command',
        implemented: false,
        category: 'Commands',
      },
      {
        id: 'refresh-movie',
        name: 'Refresh Movie',
        description: 'Refresh movie metadata from TMDB',
        method: 'POST',
        endpoint: '/command',
        implemented: false,
        category: 'Commands',
      },
      {
        id: 'rescan-movie',
        name: 'Rescan Movie',
        description: 'Scan disk for movie files',
        method: 'POST',
        endpoint: '/command',
        implemented: false,
        category: 'Commands',
      },
      {
        id: 'movies-search',
        name: 'Search Movies',
        description: 'Search indexers for movies',
        method: 'POST',
        endpoint: '/command',
        implemented: false,
        category: 'Commands',
      },
      {
        id: 'rss-sync',
        name: 'RSS Sync',
        description: 'Sync with RSS feeds',
        method: 'POST',
        endpoint: '/command',
        implemented: false,
        category: 'Commands',
      },

      // Collections
      {
        id: 'collection-list',
        name: 'List Collections',
        description: 'Get movie collections',
        method: 'GET',
        endpoint: '/collection',
        implemented: false,
        category: 'Collections',
      },
      {
        id: 'collection-get',
        name: 'Get Collection',
        description: 'Get a specific collection',
        method: 'GET',
        endpoint: '/collection/{id}',
        implemented: false,
        category: 'Collections',
      },

      // Profiles
      {
        id: 'quality-profiles',
        name: 'List Quality Profiles',
        description: 'Get quality profiles',
        method: 'GET',
        endpoint: '/qualityprofile',
        implemented: false,
        category: 'Profiles',
      },
      {
        id: 'custom-formats',
        name: 'List Custom Formats',
        description: 'Get custom formats',
        method: 'GET',
        endpoint: '/customformat',
        implemented: false,
        category: 'Profiles',
      },
      {
        id: 'delay-profiles',
        name: 'List Delay Profiles',
        description: 'Get delay profiles',
        method: 'GET',
        endpoint: '/delayprofile',
        implemented: false,
        category: 'Profiles',
      },

      // Root Folders
      {
        id: 'root-folders',
        name: 'List Root Folders',
        description: 'Get configured root folders',
        method: 'GET',
        endpoint: '/rootfolder',
        implemented: false,
        category: 'Configuration',
      },

      // Exclusions
      {
        id: 'exclusions',
        name: 'List Exclusions',
        description: 'Get import exclusions',
        method: 'GET',
        endpoint: '/exclusions',
        implemented: false,
        category: 'Configuration',
      },

      // Tags
      {
        id: 'tags',
        name: 'List Tags',
        description: 'Get all tags',
        method: 'GET',
        endpoint: '/tag',
        implemented: false,
        category: 'Configuration',
      },

      // Indexers
      {
        id: 'indexers',
        name: 'List Indexers',
        description: 'Get configured indexers',
        method: 'GET',
        endpoint: '/indexer',
        implemented: false,
        category: 'Indexers',
      },
      {
        id: 'indexer-test',
        name: 'Test Indexer',
        description: 'Test an indexer connection',
        method: 'POST',
        endpoint: '/indexer/test',
        implemented: false,
        category: 'Indexers',
      },

      // Download Clients
      {
        id: 'download-clients',
        name: 'List Download Clients',
        description: 'Get configured download clients',
        method: 'GET',
        endpoint: '/downloadclient',
        implemented: false,
        category: 'Download Clients',
      },

      // Import Lists
      {
        id: 'import-lists',
        name: 'List Import Lists',
        description: 'Get configured import lists',
        method: 'GET',
        endpoint: '/importlist',
        implemented: false,
        category: 'Import Lists',
      },

      // Notifications
      {
        id: 'notifications',
        name: 'List Notifications',
        description: 'Get configured notifications',
        method: 'GET',
        endpoint: '/notification',
        implemented: false,
        category: 'Notifications',
      },

      // Manual Import
      {
        id: 'manual-import',
        name: 'Manual Import',
        description: 'Get files for manual import',
        method: 'GET',
        endpoint: '/manualimport',
        implemented: false,
        category: 'Import',
        parameters: [
          { name: 'folder', type: 'string', required: true, description: 'Folder path' },
        ],
      },

      // Parse
      {
        id: 'parse',
        name: 'Parse Title',
        description: 'Parse a release title',
        method: 'GET',
        endpoint: '/parse',
        implemented: false,
        category: 'Utilities',
        parameters: [
          { name: 'title', type: 'string', required: true, description: 'Release title to parse' },
        ],
      },

      // Rename
      {
        id: 'rename',
        name: 'Preview Rename',
        description: 'Preview movie file renaming',
        method: 'GET',
        endpoint: '/rename',
        implemented: false,
        category: 'Utilities',
        parameters: [
          { name: 'movieId', type: 'number', required: true, description: 'Movie ID' },
        ],
      },

      // Logs
      {
        id: 'logs',
        name: 'Get Logs',
        description: 'Get log entries',
        method: 'GET',
        endpoint: '/log',
        implemented: false,
        category: 'System',
      },
    ];
  }
}
