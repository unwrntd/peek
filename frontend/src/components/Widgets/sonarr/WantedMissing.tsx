...
export function WantedMissing({ integrationId, config, widgetId }: WantedMissingProps) {
  ...
  const showAirDate = config.showAirDate !== false;
    ...
    return (
      <BaseWidget loading={loading} error={error}>
        <div className="flex flex-col items-center justify-center h-full text-gray-500 dark:text-gray-400">
          ...
          <span className="text-sm">No missing episodes</span>
        </div>
      </BaseWidget>
    );
  }
    ...
    return (
      <BaseWidget loading={loading} error={error}>
        <div className="space-y-1">
          {missing.slice(0, itemCount).map((ep) => (
            <div
              key={ep.id}
            ...
            >
              ...
              {showAirDate && ep.airDateUtc && (
                <span className="text-xs text-gray-500 dark:text-gray-400 flex-shrink-0">
                  {getDaysAgo(ep.airDateUtc)}
                </span>
              )}
            </div>
          ))}
      ...
      <div className="space-y-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {showAirDate && ep.airDateUtc && (
                    <div className="mt-2 flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                      ...
                      <span>Aired {formatDate(ep.airDateUtc)}</span>
        ...
        {data?.totalRecords && data.totalRecords > itemCount && (
          <div className="text-sm text-center text-gray-500 dark:text-gray-400">
            +{data.totalRecords - itemCount} more missing episodes
          </div>
        )}
      </div>
    </BaseWidget>
