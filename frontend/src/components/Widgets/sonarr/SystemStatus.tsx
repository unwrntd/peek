...
import { SonarrSystemStatus, SonarrDiskSpace, SonarrHealth } from '../../../types';
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between py-1.5 px-2 rounded bg-green-50 dark:bg-green-900/20">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-gray-900 dark:text-white">Sonarr v{status.version}</span>
            </div>
            <span className="text-xs text-gray-500 dark:text-gray-400">
              Up {formatUptime(status.startTime)}
            </span>
          </div>
          ...
          {showDiskSpace && diskSpace.length > 0 && (
            <div className="space-y-1">
              {diskSpace.slice(0, 3).map((disk, idx) => {
                const usedPercent = Math.round(((disk.totalSpace - disk.freeSpace) / disk.totalSpace) * 100);
                const isLow = usedPercent > 90;
                return (
                  <div key={idx} className="flex items-center justify-between py-1 px-2 rounded bg-gray-50 dark:bg-gray-700/50">
                    <span className="text-xs text-gray-600 dark:text-gray-400 truncate max-w-[120px]">
                      {disk.path}
                    </span>
                    <span className={`text-xs ${isLow ? 'text-red-600 dark:text-red-400' : 'text-gray-500 dark:text-gray-400'}`}>
          ...
          {showHealth && health.length > 0 && (
            <div className="space-y-1">
              {health.slice(0, 3).map((issue, idx) => (
                <div key={idx} className={`py-1 px-2 rounded text-xs ${getHealthColor(issue.type)}`}>
                  {issue.message}
                </div>
              ))}
            </div>
  ...
  return (
    <BaseWidget loading={loading} error={error}>
      <div className="space-y-4">
        {/* System Info Card */}
        <div className="p-3 rounded-lg bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 border border-green-100 dark:border-green-800/50">
