...
import { SonarrHistoryRecord } from '../../../types';
...
export function ActivityHistory({ integrationId, config, widgetId }: ActivityHistoryProps) {
  ...
  const eventFilter = (config.eventFilter as string) || '';
  ...
  const timelineEvents: TimelineEvent[] = records.slice(0, itemCount).map(record => {
    const episodeInfo = record.episode
      ? `S${record.episode.seasonNumber.toString().padStart(2, '0')}E${record.episode.episodeNumber.toString().padStart(2, '0')}`
      : '';
    return {
      id: String(record.id),
      ...
      <BaseWidget loading={loading} error={error}>
        <div className="space-y-1">
          {records.slice(0, itemCount).map((record) => (
            <div
              key={record.id}
              className={`flex items-center justify-between py-1.5 px-2 rounded ${getEventColor(record.eventType)}`}
            >
              <div className="flex items-center gap-2 min-w-0">
                <span className="flex-shrink-0">{getEventIcon(record.eventType)}</span>
                <span className="text-sm truncate">
                  <span className="font-medium">{record.series?.title || 'Unknown'}</span>
                  {record.episode && (
              ...
              <span className="text-xs flex-shrink-0 opacity-80">
                  ...
                  <div>
                    ...
                    {record.episode && (
                      <p className="text-sm text-gray-600 dark:text-gray-400">
                        S{record.episode.seasonNumber.toString().padStart(2, '0')}E{record.episode.episodeNumber.toString().padStart(2, '0')} - {record.episode.title}
                      </p>
                    )}
                  </div>
