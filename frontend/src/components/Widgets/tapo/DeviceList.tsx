...
import { TapoDevice, TapoDeviceType } from '../../../types';
...
function getDeviceTypeName(type: TapoDeviceType): string {
  switch (type) {
    case 'plug':
      return 'Smart Plug';
    case 'plug_energy':
...
export function DeviceList({ integrationId, config, widgetId }: DeviceListProps) {
  ...
  const filteredDevices = data?.devices.filter(device => {
    ...
    if (statusFilter === 'on' && !device.deviceOn) {
      return false;
    }
    if (statusFilter === 'off' && device.deviceOn) {
      return false;
    }
    return true;
  }) || [];
        ...
        <div className={`${compactView ? 'space-y-1' : 'space-y-2'}`}>
          {filteredDevices.map(device => (
            ...
            >
                ...
                <div className="min-w-0">
                  ...
                  {!hideLabels && (
                    <div className="text-xs text-gray-500 dark:text-gray-400">
                      {device.deviceModel} - {getDeviceTypeName(device.deviceType)}
                    </div>
                  )}
                </div>
              </div>
              <div className="flex items-center gap-2">
                ...
                {device.ipAddress ? (
                  // Status known from local connection
                  <div
                    ...
                    title={device.deviceOn ? 'On' : 'Off'}
                  />
                ) : (
                  // Status unknown (cloud-only mode)
                  <div
                    className="w-3 h-3 rounded-full bg-gray-300 dark:bg-gray-600 ring-1 ring-gray-400 dark:ring-gray-500"
                    title="Status unknown (add device IP for live status)"
                  />
                )}
              </div>
            </div>
          ...
          {!hideLabels && filteredDevices.length > 0 && (
            <div className="text-xs text-gray-500 dark:text-gray-400 text-center pt-2 border-t border-gray-200 dark:border-gray-700">
              {filteredDevices.some(d => d.ipAddress)
                ? `${filteredDevices.filter(d => d.deviceOn).length} on / ${filteredDevices.length} total`
